...
void BornAgainDecisionTree::buildOptimal()
{
	...
	// Initialize the region where we compute the BA_tree
	int indexBottom; 
	int indexTop;
	int currentIndex;
	...
	//else indexBottom = fspaceFinal.cellToKey(params->BottomL);

	//if ((params->TopR).empty()) indexTop = (int)fspaceFinal.nbCells - 1;
	//else  indexTop = fspaceFinal.cellToKey(params->TopR); 
	...
	if (params->objectiveFunction == 0)      finalObjective = dynamicProgrammingOptimizeDepth(indexBottom, indexTop);
	else if (params->objectiveFunction == 1) finalObjective = dynamicProgrammingOptimizeNbSplits(indexBottom, indexTop);
	else if (params->objectiveFunction == 2) finalObjective = dynamicProgrammingOptimizeDepthThenNbSplits(indexBottom, indexTop);
	else throw std::string("NON RECOGNIZED OBJECTIVE");

	// Collect the final solution
	collectResultDP(indexBottom, indexTop, finalObjective, 0);
}
...
void BornAgainDecisionTree::exportRunStatistics(std::string fileName)
{
	...
	//	left.push_back(fspaceFinal.keyToCell(10,i));
	//	right.push_back(fspaceFinal.keyToCell(fspaceFinal.nbCells-10,i));
	//}
	...
	if (myfile.is_open())
	{
		...
		//for (int j : left) myfile << j << ",";
		//myfile << endl;
		//for (int k : right) myfile << k << ",";
		//myfile << endl;
		myfile.close();
	}
	else
